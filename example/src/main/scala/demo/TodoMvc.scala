...
import csw.sjreact.{EventChannel, State, StateMod, Var, _}
...
object TodoMvc {
  ...
  def run(): Unit = {
    val rootView = view { implicit manager =>
      ...
      implicit val channel: EventChannel[ToDoEvent] = EventChannel[ToDoEvent] {
        case Add(str) => allItems.update(_ :+ TodoItem(str))
        case ChangeState(item) =>
          allItems.updateItem[TodoItem](item, i => i.copy(complete = !i.complete))
